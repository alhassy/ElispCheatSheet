...
#+end_parallel
  ...
  # More honestly, Elisp has distinct namespaces for functions and variables.
...
#+BEGIN_SRC emacs-lisp
;; Generally:
  (f)           ;; calls global function f
  (funcall f)   ;; calls function bound to f
  (funcall #'f) ;; calls global function f
#+END_SRC
:End:
...
** TODO COMMENT more #' notes

    Three ways to define anonymous functions: ~function, lambda, #'~.

 0. Plain quote, ~'~, tells the compiler to leave its argument as an expression, no further compilation is possible.

 1. ~function~ behaves like ~quote~ but tells the compiler its argument is intended to be
    used as a function and should be compiled as such or converted into a closure.

 2. ~lambda~ uses ~function~ as a subroutine to tell the compiler its argument is a function.

 3. Thus, lambdas are self-quoting: Evaluating a form whose ~car~ is a ~lambda~ yields
    the form itself. ( It yields a closure under lexical binding. )

 4. The /sharp quote/ ~#'~ is short-hand for ~function~.
...
(macroexpand '#'(lambda (x) x) ;; ⇒ (function (lambda (x) x))
(macroexpand ''(lambda (x) x))  ;; ⇒ (function (lambda (x) x))

''(lambda (x) x) ;; A list of items: Literal lambda, List (x), Literal x
;; TODO: MA: New adventure: When do we want to use quote-quote ?

(funcall ''(lambda (x) (+ x 2)) 3)

;; Whence there is never any need to quote lambdas with either quote.
 #+end_src

* TODO COMMENT interactive
...
(with-temp-buffer
    ...
    "#+EXPORT_FILE_NAME: README.md
     ...
     ◈ [[https://alhassy.github.io/ElispCheatSheet/][Website]] ◈
